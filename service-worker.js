...
self.addEventListener('activate',event=>event.waitUntil((async()=>{
  ...
  clients.forEach(c=>c.postMessage({type:'APP_UPDATED',version:VERSION}));
})()));
  ...
  if(event.request.mode==='navigate'){
    event.respondWith((async()=>{
      ...
      try{
        ...
        return await pageResponse(response);
      }catch(e){
        const cached=await cache.match('./index.html');
        return cached?await pageResponse(cached):new Response('Sem conexão. Conecte-se à internet para abrir o app.',{status:503,headers:{'Content-Type':'text/plain; charset=utf-8'}});
      }
    })());return;
  }
...
self.addEventListener('push',event=>{
  let data={title:'Meu Financeiro',body:''};
  try{data=event.data?.json()||data}catch(e){data.body=event.data?.text()||''}
