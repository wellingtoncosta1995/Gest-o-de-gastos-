const CACHE='gestao-gastos-v30';
const ASSETS=['./','./index.html','./pierre-layout.css?v=30','./ui-modern-v2.js?v=30','./manifest.webmanifest','./icon-192.png','./icon-512.png'];
...
function adicionarLayout(response){return response.text().then(html=>{
  ...
  const patch='<link rel="stylesheet" href="./pierre-layout.css?v=30">\n<script src="./ui-modern-v2.js?v=30" defer></script>\n';
...
self.addEventListener('fetch',event=>{if(event.request.method!=='GET')return;const url=new URL(event.request.url);const isPage=event.request.mode==='navigate'||url.pathname.endsWith('/index.html');const isModernAsset=url.pathname.endsWith('/pierre-layout.css')||url.pathname.endsWith('/ui-modern-v2.js');if(isPage){event.respondWith(fetch(event.request,{cache:'no-store'}).then(response=>adicionarLayout(response.clone())).catch(()=>caches.match('./index.html').then(cached=>adicionarLayout(cached))));return}if(isModernAsset){event.respondWith(fetch(event.request,{cache:'no-store'}).then(response=>{const copy=response.clone();caches.open(CACHE).then(cache=>cache.put(event.request,copy));return response}).catch(()=>caches.match(event.request)));return}event.respondWith(caches.match(event.request).then(cached=>cached||fetch(event.request)))});
