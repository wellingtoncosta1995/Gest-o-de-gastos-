const CACHE='gestao-gastos-v42';
const ASSETS=['./','./index.html','./pierre-layout.css?v=40','./ui-modern-v2.js?v=34','./ui-home-v39.js?v=39','./ui-home-guard-v41.js?v=41','./ui-modal-fix-v42.js?v=42','./logo-meu-financeiro.svg?v=36','./app-icon.svg?v=35','./manifest.webmanifest','./icon-192.png','./icon-512.png'];
...
function adicionarLayout(response){return response.text().then(html=>{
  ...
  html=html.replace(/<script[^>]*src=["'][^"']*ui-fix-v36\.js[^"']*["'][^>]*><\/script>\s*/gi,'');
  html=html.replace(/<script[^>]*src=["'][^"']*ui-home-v37\.js[^"']*["'][^>]*><\/script>\s*/gi,'');
  html=html.replace(/<script[^>]*src=["'][^"']*ui-home-fix-v38\.js[^"']*["'][^>]*><\/script>\s*/gi,'');
  html=html.replace(/<script[^>]*src=["'][^"']*ui-home-v39\.js[^"']*["'][^>]*><\/script>\s*/gi,'');
  html=html.replace(/<script[^>]*src=["'][^"']*ui-home-guard-v41\.js[^"']*["'][^>]*><\/script>\s*/gi,'');
  html=html.replace(/<script[^>]*src=["'][^"']*ui-modal-fix-v42\.js[^"']*["'][^>]*><\/script>\s*/gi,'');
...
self.addEventListener('fetch',event=>{if(event.request.method!=='GET')return;const url=new URL(event.request.url);const isPage=event.request.mode==='navigate'||url.pathname.endsWith('/index.html');const isModernAsset=url.pathname.endsWith('/pierre-layout.css')||url.pathname.endsWith('/ui-modern-v2.js')||url.pathname.endsWith('/ui-home-v39.js')||url.pathname.endsWith('/ui-home-guard-v41.js')||url.pathname.endsWith('/ui-modal-fix-v42.js')||url.pathname.endsWith('/logo-meu-financeiro.svg')||url.pathname.endsWith('/app-icon.svg');if(isPage){event.respondWith(fetch(event.request,{cache:'no-store'}).then(response=>adicionarLayout(response.clone())).catch(()=>caches.match('./index.html').then(cached=>adicionarLayout(cached))));return}if(isModernAsset){event.respondWith(fetch(event.request,{cache:'no-store'}).catch(()=>caches.match(event.request)));return}event.respondWith(caches.match(event.request).then(cached=>cached||fetch(event.request)))});
