...
self.addEventListener('activate',event=>event.waitUntil((async()=>{const keys=await caches.keys();await Promise.all(keys.filter(k=>k.startsWith(PREFIX)&&k!==CACHE).map(k=>caches.delete(k)));await self.clients.claim();const clients=await self.clients.matchAll({type:'window',includeUncontrolled:true});clients.forEach(c=>c.postMessage({type:'APP_UPDATED',version:VERSION}))})()));
self.addEventListener('message',event=>{if(event.data?.type==='GET_APP_VERSION')event.source?.postMessage({type:'APP_VERSION',version:VERSION})});
...
self.addEventListener('fetch',event=>{
  ...
  const known=ASSETS.some(a=>new URL(a,self.registration.scope).pathname===url.pathname)||CDN.has(url.href);
  if(!known)return;
