/* Meu Financeiro v30 — correção definitiva do perfil. */
(function(){
  ...
  const logo=`<div class="app-brand" aria-label="Meu Financeiro"><div class="mf-logo-icon" aria-hidden="true"><svg viewBox="0 0 72 72"><defs><linearGradient id="mf30" x1="0" x2="1" y1="1" y2="0"><stop stop-color="#174A60"/><stop offset="1" stop-color="#36B97A"/></linearGradient></defs><path d="M9 54V29l14 14 13-20v31h9V13l8 8V7l15 15v32H55V39L43 54H31L23 44 9 54Z" fill="url(#mf30)"/></svg></div><div class="brand-copy"><b>MEU FINANCEIRO</b><span>CONTROLE INTELIGENTE</span></div></div>`;
  ...
  function moveProfile(){
    ...
    let profile=header.querySelector('.icon-btn');
    if(!profile) profile=header.querySelector('button, img');
    if(!profile) return false;
    let wrap=document.querySelector('.profile-outside-card');
    if(!wrap){
      wrap=document.createElement('div');
      wrap.className='profile-outside-card';
      header.parentNode.insertBefore(wrap,header);
    }
    if(profile.parentElement!==wrap) wrap.appendChild(profile);
    return true;
  }
  ...
  function enhance(){
    addBrand();
    moveProfile();
    document.querySelectorAll('.top-header .icon-btn, .top-header .profile-btn, .top-header [data-profile]').forEach(el=>{
      if(!document.querySelector('.profile-outside-card')) return;
      document.querySelector('.profile-outside-card').appendChild(el);
    });
  ...
  [100,300,700,1200,2000].forEach(t=>setTimeout(enhance,t));
